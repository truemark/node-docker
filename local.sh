export NODE_VERSION=$(curl -sSL https://nodejs.org/dist/latest-v20.x/ | grep linux-x64.tar.gz | sed -n "s/.*>node-v\(.*\)-linux.*<.*/\1/p")
echo "Using node version ${NODE_VERSION}"

docker build -t moo --build-arg OS_NAME=ubuntu --build-arg OS_VERSION=noble --build-arg NODE_VERSION=${NODE_VERSION} -f Dockerfile .
